...
class MediaWiki2DokuWiki_Environment
{
    ...
    public function __construct(array $settings)
    {
        ...
        try {
            ...
            $converter->convert($db,"dry");

            //var_dump(MediaWiki2DokuWiki_MediaWiki_Converter::$idsOfGroups);
            //var_dump(MediaWiki2DokuWiki_MediaWiki_Converter::$customNamespaces);
            //exit();
            $converter->convert($db,"files");
            //var_dump(MediaWiki2DokuWiki_MediaWiki_Converter::$idsOfGroups);
            //var_dump(MediaWiki2DokuWiki_MediaWiki_Converter::$filesOfGroups);
            //exit();


            $converter->convert($db,"images");

            //var_dump(MediaWiki2DokuWiki_MediaWiki_Converter::$idsOfGroups);

            //var_dump("MediaWiki2DokuWiki_MediaWiki_Converter::linksOfGroups");
            //var_dump(MediaWiki2DokuWiki_MediaWiki_Converter::$idsOfGroups);
            //var_dump(MediaWiki2DokuWiki_MediaWiki_Converter::$idsOfCategories);
            //var_dump(MediaWiki2DokuWiki_MediaWiki_Converter::$filesOfGroups);

            if(!empty($settings['useStaticPages'])){
             foreach($settings['useStaticPages'] as $staticPageName){
                 $converter->saveStaticPage($staticPageName);
             }
            }
            //$converter->saveStaticPage("Hlavni_strana");
            //$converter->saveStaticPage("sidebar");
        } catch (Exception $e) {
            $this->out($e->getMessage());
            return;
        }
